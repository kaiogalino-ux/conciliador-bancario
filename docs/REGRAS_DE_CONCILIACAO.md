...
3. **Desempate por nome/descrição** — quando há mais de um lançamento com o mesmo valor e a mesma data, mas descrições diferentes que permitem saber quem é quem (ex.: nome da pessoa aparece nos dois lados, mesmo que abreviado — "SILV" é reconhecido como "SILVA" truncado) → concilia cada par certo individualmente; pares de datas diferentes nunca entram nesse desempate.
...
5. **Lote de salário/férias/rescisão (NET EMP)** — o **banco** de um lote NET EMP/EMPR nunca é comparado um a um; mas um lançamento do **ERP** desse tipo tenta primeiro a via individual normal, sempre por valor e mesma data — só entra na etapa de lote se não encontrar nenhum par seguro.
6. **Data exata obrigatória** — a tolerância é zero para todos os lançamentos. ERP e banco só podem ser conciliados quando as datas forem iguais. Mesmo valor e nome forte em datas diferentes não conciliam; o ERP vai para Revisão Manual com o motivo de que a conciliação exige a mesma data, e o lançamento bancário permanece como Somente banco.
...
- Qualquer possível par encontrado em data diferente, independentemente de nome ou valor compatíveis.
