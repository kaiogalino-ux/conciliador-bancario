...
Roda 100% por terminal/VS Code (`python main.py`), sem interface gráfica. Não é um produto para terceiros — é uma ferramenta interna meu/minha equipe usa para fechar a conciliação bancária mensal sem fazer isso manualmente linha a linha numa planilha.
...
- Sem banco de dados, sem API própria, sem front-end. Entrada = arquivos em pastas; saída = um `.xlsx`.
...
├── resultado/
│   └── Resultado.xlsx  -> gerado a cada execução, ABA ÚNICA "Resultado"
├── logs/
...
## Como funciona (fluxo do `main.py`)

1. Lê o Excel mais recente de `dados/ERP/`.
2. Lê o extrato mais recente de `dados/Banco/`.
3. Roda a conciliação (`src/conciliador.py`), com a camada de IA opcional por cima.
4. Exporta `resultado/Resultado.xlsx`.

Para rodar: ativar o `.venv`, `pip install -r requirements.txt`, depois `python main.py` (interpretador `.venv` selecionado no VS Code).
...
Suíte `pytest` em `tests/` protege todas as regras acima com dados sintéticos (não depende dos arquivos reais em `dados/`). **Sempre rodar `pytest` antes e depois de qualquer alteração em `src/`.**
...
- Depois de qualquer mudança em `src/`, rodar `pytest` antes de considerar a tarefa concluída.
- Ao rodar `python main.py`, sempre reportar o checklist completo: quantos lançamentos lidos do ERP/banco, quantos débitos/créditos, quantos conciliados por cada regra, quantos em Revisão Manual, Não encontrado, Somente banco (checklist completo em `CLAUDE.md`).
